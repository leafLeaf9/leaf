...
(function($){
	...
	view = {
		...
		makeDOMNodeIcon: function(html, setting, node) {
			...
			tip = view.makeNodeTip(setting, node);
		...
		makeNodeTip: function(setting, node) {
			var tipKey = setting.data.key.tip;
			// tip 值为 0 的时候处理为 标签 new
			if(node[tipKey] == 0) {
				return 0;
			}
			return node[tipKey] ? node[tipKey] : null;
		},
		makeNodeResourceType: function(setting, node) {
			var resourceKey = node.resourceType;
			return node.resourceType ? node.resourceType : null;
		},
		makeNodeIco: function(setting, node) {
			var icoKey = setting.data.key.ico;
			return node[icoKey] ? node[icoKey] : null;
		},
		makeChildNode: function( node) {
		...
		setNodeTip: function(setting, node) {
			var aObj = $$(node, consts.id.A, setting),
			tip = view.makeNodeTip(setting, node);
