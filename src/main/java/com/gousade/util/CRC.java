...
public class CRC {
    ...
    public static String CRC16(int[] byteCRC) {

        byte uchCRCHi = (byte) 0xFF; /* 高CRC字节初始化 */
        byte uchCRCLo = (byte) 0xFF; /* 低CRC 字节初始化 */
        byte uIndex; /* CRC循环中的索引 */
        int l = byteCRC.length;
        int i = 0;

        while (i < l) /* 传输消息缓冲区 */ {
            uIndex = (byte) (uchCRCLo ^ (byte) (byteCRC[i] & 0xFF)); /* 计算CRC */
            uchCRCLo = (byte) (uchCRCHi ^ (byte) (auchCRCHi[(uIndex & 0xFF)] & 0xFF));
            uchCRCHi = (byte) (auchCRCLo[(uIndex & 0xFF)] & 0xFF);
            i++;
        }
        String tempStr = String.format("%04x", ((uchCRCHi << 8) & 0xFFFF)
                | (uchCRCLo & 0x00FF));
